...
wb = load_workbook("Notes_only_uploader_tool_metadata_v4_raw_reads_sample_only.xlsx")
ws = wb.worksheets[0]
...
if ws['K6'].value !=None:
    doc.asis(xml_header)
    with tag('SAMPLE_SET'):
        for row in ws.iter_rows(min_row=6, min_col=11, max_col=38, values_only=True):
            found = False
            for x in row:
                if x != None:
                    found = True
                        ...
                        for y in range(len(all)):
                            ...
                            if x != None:
                                with tag("SAMPLE_ATTRIBUTE"):
                                    ...
                                    with tag("VALUE"):
                                        text(str(x))
                                    if ws[5][y+10+len(first)].value != None:
                                        with tag("UNITS"):
                                            text(ws[5][y+10+len(first)].value)
                        with tag("SAMPLE_ATTRIBUTE"):
                            with tag("TAG"):
                                text("ENA-CHECKLIST")
                            with tag("VALUE"):
...
with tag('SUBMISSION_SET'):
    with tag('SUBMISSION'):
        with tag("ACTIONS"):
            ...
            if ws['I6'].value != None:
                with tag('ACTION'):
                    doc.stag('HOLD', HoldUntilDate=str(ws['I6'].value))
