...
for f in files_xlsx:
    if fnmatch.fnmatch(f, '*genome*'):
        ...
        print(df)
        df.to_excel(r'trimmed_assembly_study_sample_metadata_27_jan.xlsx', index=False)
        ...
        print('user sample & study data below:')
        print(df)
        df.to_excel(r'trimmed_raw_reads_study_sample_metadata_27_jan.xlsx', index=False)
    else:
        print('you have used an unsupported spreadsheet, please try again')


wb = load_workbook("trimmed_raw_reads_study_sample_metadata_27_jan.xlsx")
ws = wb.worksheets[0]
